...
$$G \cap Y = \bigcup_{x \in A} \left(B_{r_x}^X(x) \cap Y\right) = \cup_{x \in A} B_{r_x}^Y(x) = A$$
$\La$:
...
Но тогда
$$x \notin \cl A$$
$\La$: Пусть $x \notin \cl A \Ra \exists F \supset A: x \notin F \And F \text{ открыто}$
Тогда
$$x \in X \subset F \text{ --- открытое} \Ra \exists r > 0\colon B_r(x) \subset X \setminus F \Ra B_r(x) ... $$
\end{proof}
\begin{conseq}
$ U \text{ открытое } \And U \cap A = \varnothing \Ra U \cap \cl A = \varnothing$
\end{conseq}
\begin{proof}
...
$\subset$: Пусть $x \in (A \cup B)' \And x \notin B'$.
...
\end{theorem}
\begin{proof}
$$x \in A' \Ra \exists \dot B_r(x) \cap A \ne \varnothing \Ra \exists y_1 \in A\colon y_1 \ne x \And y \in B_r(x)$$
Тогда
$$\exists \dot B_{\rho(x,y_1)} \cap A \ne \varnothing \Ra \exists y_2 \in A\colon y_2 \ne x \And y_2 \ne y_1 \And y \in B_{\rho(x,y_1)}$$
Тогда рассмотрим
...
\begin{proof}
$a = \sup A$. Тогда
$$\forall x \in A x \leqslant a \And \forall \varepsilon > 0\; \exists x \in A\colon x > a - \varepsilon$$
Пусть $a \notin A \Ra$. Рассмотрим $\dot B_r(a) = (a - r, a + r)$.
...
$$x^* = \lim_{n\ra\infty} x_n \LraDef x^* \in X \And \forall \varepsilon > 0\; \exists N\colon \forall n \geqslant N\; \rho(x^*;x_i) < \varepsilon$$
\end{Def}
Примеры:
\begin{itemize}
...
\begin{enumerate}
\item $x^* = \lim_{n\ra\infty} x_n \Lra \text{каждая окрестность } x^* \text{ содержит всю последовательность с некотрого элемента}$
\item $x^* = \lim_{n\ra\infty} x_n \And x^{**} = \lim_{n\ra\infty} \Ra x^* = x^{**}$
...
$$\forall \varepsilon > 0\; \exists N\colon \forall n \geqslant N\; \rho(x^*;x_n) < \varepsilon \Ra$$
$$\Ra \exists N\colon \forall n \geqslant N\; x_n \in U$$
$\La$: $U \lrh B_\varepsilon(x^*)$.
$$\forall \varepsilon > 0\; \exists N\colon \forall n \geqslant N\; x_n \in U \Ra x_* = \lim_{n\ra\infty} x_n$$
\item Пусть $\varepsilon \lrh \frac{\rho(x^*;x^{**})}2 > 0$
$$x^* = \lim_{n\ra\infty} x_n \Ra \exists N_1\colon \forall n \geqslant N_1\; \rho(x^*;x_n) < \varepsilon$$
$$x^{**} = \lim_{n\ra\infty} x_n \Ra \exists N_2\colon \forall n \geqslant N_2\; \rho(x^{**};x_n) < \varepsilon$$
Тогда
$$\forall n \geqslant \max\{N_1; N_2\} \left\{\begin{aligned}\rho(x^*;x_n) < \varepsilon \\ \rho(x^{**};x_n) < \varepsilon\end{aligned}\right. \Ra$$
$$\Ra 2\varepsilon = \rho(x^*;x^{**}) \leqslant \rho(x^*;x_n) + \rho(x^{**}; x_n) < 2\varepsilon$$
...
\end{Rem}
\begin{Rem}
$$x = \lim_{n\ra\infty} x_n \And x_n \in A \Ra x \in \cl A$$
\end{Rem}
...
\end{theorem}
\begin{proof}
Пусть $y < x$; $\varepsilon \lrh \frac{x - y}2$. Тогда 
$$\exists N_1: \forall n \geqslant N_1\; |x - x_n| < \varepsilon$$
$$\exists N_2: \forall n \geqslant N_2\; |y - y_n| < \varepsilon$$
Тогда
$$\forall n \geqslant \max\{N_1, N_2\}\; x_n > x - \varepsilon = y + \varepsilon > y_n$$
\end{proof}
\begin{Rem}
...
\end{theorem}
\begin{proof}
Выберем $\varepsilon > 0$.
$$\exists N_1\colon\forall n \geqslant N_1 x_n > l - \varepsilon$$
$$\exists N_2\colon\forall n \geqslant N_2 z_n < l + \varepsilon$$
Тогда
$$\exists N = \max\{N_1,N_2\}\colon\forall n \geqslant N\;l - \varepsilon < x_n \leqslant y_n \leqslant z_n < l + \varepsilon$$
Тогда $\lim y_n = l$
\end{proof}
\begin{conseq}
$\lim z_n = 0 \And |y_1| \leqslant z_n \Ra \lim y_n = 0$
\end{conseq}
\begin{conseq}
...
\begin{Def}
$(x_n)$ нестрого монотонно возрастает, если $x_1 \leqslant x_2 \leqslant x_3 \leqslant \ldots$. \\
$(x_n)$ строго монотонно возрастает, если $x_1 < x_2 < x_3 < \ldots$.   \\
$(x_n)$ нестрого монотонно возрастает, если $x_1 \leqslant x_2 \leqslant x_3 \leqslant \ldots$. \\
$(x_n)$ строго монотонно возрастает, если $x_1 > x_2 > x_3 > \ldots$.   \\
\end{Def}
...
\begin{proof}
$\Ra$: Очевидно. \\
$\La$: Пусть $(x_n)$ возрастает. Она ограниченна, значит есть супремум. Докажем, что это и есть предел. Возьмём $\varepsilon > 0$.
$$a = \sup \{x_n\} \Ra \exists x_k\colon x_k > x - \varepsilon \Ra a - \varepsilon < x_k \leqslant x_{k+1} \leqslant \ldots \leqslant a$$
Тогда $$\forall n \geqslant k\; |x_n - a| < \varepsilon$$
\end{proof}