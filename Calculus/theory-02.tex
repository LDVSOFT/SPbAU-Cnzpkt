...
\begin{Def}
Пусть есть множество $X$ и 2отображение $\rho \colon X \times X \ra \left[0; +\infty\right) $. Тогда $\rho$ называется метрикой, если:
\begin{enumerate}
...
2\left(\underbrace{a_1b_1 + \ldots + a_nb_n}_{\lrh C}\right)t + $$
$$ +\left(\underbrace{b_1^2 + \ldots + b_2^2}_{\lrh B}\right)$$
$$f \text{ имеет не более 1 корня} \Ra (2С)^2 - 4AB \leqslant 4\left(C^2 - 4AB\right) \geqslant 0 \Lra C^2 \leqslant 0$$
Можно считать, что все числа не равны 0 --- cслишком просто.
\begin{Rem}
...
\begin{proof}
Возведём в квадрат
$$ \Lra \sum_{i=1}^n (a_i + b_i)^2 \leqslant A + 2\sqrt{AB} + B \Lra$$
$$ \Lra A + B + 2\sum_{i=1}^n a_ib_i \Lra A + B + 2\sqrt{AB} \Lra$$
$$ \Lra \sum_{i=1}^n a_ib_i \leqslant \sqrt{AB} \Ra$$
$$ \Ra \text{Неравенство Коши-Буняковского}$$
\begin{Rem}
Равентсво в случае, если числа пропорциональны.
\end{Rem}
\end{proof}
...
\item $x \ne y \Ra \exists r > 0\colon B_r(x) \cap \colon B_r(y) = \varnothing$
\begin{proof}
...
\item $$x \in \bigcap_{k=1}^n G_k \forall k=1..n\; x \in G_k \Ra \forall k=1..n\; \exists r_k > 0 \colon B_{r_k}(x) \in G_k \Ra $$
$$ \Ra \exists r = \min{r_k}\colon G_r \in \bigcap_{k=1}^n G_k$$
